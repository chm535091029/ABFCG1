...
class SimpleRetention(nn.Module):
    def __init__(self, hidden_size, gamma, head_size=None, double_v_dim=False):
        ...
        self.W_Q = nn.Parameter(torch.randn(hidden_size, head_size) / hidden_size).to(device)
        self.W_K = nn.Parameter(torch.randn(hidden_size, head_size) / hidden_size).to(device)
        self.W_V = nn.Parameter(torch.randn(hidden_size, self.v_dim) / hidden_size).to(device)
    ...
    def forward_chunkwise(self, x_i, r_i_1, i):
        ...
        D = self._get_D(chunk_size).to(device)
